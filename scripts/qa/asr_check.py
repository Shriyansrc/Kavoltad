...
def norm(s):
    return re.sub(r"[^a-z0-9 ]", "", s.lower().replace("whatsapp", "whats app")).split()
...
def main():
    ...
    out = []
    for src in ["audio/Kavolt_Kavey_20s_Narration.wav", "audio/Kavolt_Kavey_20s_Mix.wav"]:
    ...
    json.dump(out, open("audio/asr-check.json", "w"), indent=2)
