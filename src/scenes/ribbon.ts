...
export type RibbonKeys = {sweep: number; cross: number; coverStart: number; swap: number; reveal: number};
export type RibbonBody = {rot: number; height: number; energy: number};
...
/** Build the scarf wisp + lens-crossing band for one performance. */
export const makeRibbon = (K: RibbonKeys, tipAt: (f: number) => Pt, bodyAt: (f: number) => RibbonBody) => {
  /** Origin of the transition axis: where the scarf tip is at frame 990. */
  const axisOrigin = () => tipAt(K.cross);
  ...
  const scarfWisp = (f: number): RibbonShape | null => {
  ...
  const transitionBand = (f: number): RibbonShape | null => (f >= K.cross && f < K.reveal ? band(f) : null);
  return {scarfWisp, transitionBand, axisOrigin};
};

const FILM = makeRibbon(K, (f) => anchorAt(f, 'scarfTip'), (f) => kaveyPose(f));

/** Scarf-attached wisp (layer with Kavey). Hidden while the band carries it. */
export const scarfWisp = FILM.scarfWisp;
/** The lens-crossing band (transition layer, above everything). */
export const transitionBand = FILM.transitionBand;
const axisOrigin = FILM.axisOrigin;
