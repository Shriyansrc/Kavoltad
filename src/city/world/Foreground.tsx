...
export const StringLights: React.FC<{f: number}> = ({f}) => {
  ...
  for (let x0 = -2400; x0 < 5200; x0 += span) {
    ...
    spans.push(<polyline key={`c${x0}`} points={pts.join(' ')} fill="none" stroke="#050409" strokeWidth={5} />);
    for (let k = 1; k < 12; k++) {
      const x = x0 + (k / 12) * span;
      const by = sagY(x, x0, x1, y, sag) + 16;
      const tint = k % 3 === 0 ? PALETTE.magenta : k % 3 === 1 ? '#FFE9C7' : PALETTE.violet;
      const tw = 0.75 + 0.25 * Math.sin(f * 0.07 + k * 1.7 + x0);
      const on = city > 0.5 ? 1 : 0.55 + 0.45 * tw;
      spans.push(
        <g key={`b${x0}-${k}`}>
          <circle cx={x} cy={by} r={30} fill={rgba(tint, 0.1 * on)} />
          <circle cx={x} cy={by} r={9} fill={tint} opacity={0.85 * on} />
          <line x1={x} y1={by - 16} x2={x} y2={by - 8} stroke="#050409" strokeWidth={3} />
        </g>,
      );
    }
  }
  return (
    <Layer f={f} p={P_NEAR} style={{filter: 'blur(1.6px)', opacity: show}}>
      {spans}
    </Layer>
  );
};
...
export const NearPoles: React.FC<{f: number}> = ({f}) => {
  ...
  return (
    <Layer f={f} p={P_NEAR} style={{filter: 'blur(3px)'}}>
      {xs.map((x) => (
        <g key={x}>
          <rect x={x - 26} y={-400} width={52} height={2600} fill="#06050B" />
          <rect x={x - 26} y={-400} width={10} height={2600} fill={rgba(PALETTE.violet, 0.25)} />
          <rect x={x - 70} y={900} width={140} height={200} rx={10} fill="#0B0914" />
          <rect x={x - 56} y={920} width={112} height={60} rx={6} fill={rgba(PALETTE.magenta, 0.5)} />
        </g>
      ))}
    </Layer>
  );
...
export const Bokeh: React.FC<{f: number; p: number; count: number; salt: number; blurPx: number; alpha: number}> = ({f, p, count, salt, blurPx, alpha}) => {
  const motes = useMemo(() => {
    ...
    return Array.from({length: count}, (): Mote => {
      const t = rnd();
      return {x: -1400 + rnd() * 6400, y: -600 + rnd() * 2600, r: 8 + rnd() * 26, tint: t < 0.4 ? PALETTE.magenta : t < 0.75 ? PALETTE.violet : t < 0.85 ? PALETTE.cyan : '#FFE9C7', ph: rnd() * 6.28, sp: 0.3 + rnd() * 0.8};
    });
  }, [count, salt]);
